    ...
    for i, chartnumber in enumerate(chartnumbers):
        ...
        try:
            ...
            if ts[ind] > 0.01 or (i % 10) == 0:
                ...
                print('{:03}/{:03}: Added {} to set. ~{:.0f}s left.'.format(i + 1, n_items, chartnumber, time_left))
        except:
            pass
...
def chart_difference(base_from_year, base_from_week, base_to_year, base_to_week, compare_from_year, compare_from_week, compare_to_year, compare_to_week):
    ...
    return compare_set - base_set
    

if __name__ == '__main__':

    new_songs_by_artist = list(chart_difference(base_from_year, base_from_week, base_to_year, base_to_week, compare_from_year, compare_from_week, compare_to_year, compare_to_week))
    new_songs_by_artist = sorted(new_songs_by_artist, key=lambda x : x[0])
